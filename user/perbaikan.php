...
<div>
  ...
  <div class="card card-mb">
    <h3 class="section-title-icon">
      <i class="bi bi-info-circle-fill"></i> Laporan Fasilitas Umum Terbuka
    </h3>
    
    <div class="flex-col gap-12">
      <?php if (empty($publicRepairs)): ?>
        <div class="td-empty" style="padding:20px 0;">Tidak ada laporan fasilitas umum yang terbuka</div>
      <?php else: ?>
        <?php foreach ($publicRepairs as $pr): ?>
          ...
          <div class="public-repair-item">
            <div>
              <div class="pr-target"><?= htmlspecialchars($pr['target']) ?></div>
              <div class="pr-issue"><?= htmlspecialchars($pr['issue']) ?></div>
              <div class="pr-meta">
                <i class="bi bi-people-fill"></i> <span><?= $votesCount ?></span> orang telah melaporkan
              </div>
            </div>
            
            <?php if ($hasVoted): ?>
              <button class="btn btn-secondary btn-sm pr-vote-btn voted" disabled>
                <i class="bi bi-check-lg"></i> Sudah Didukung
              </button>
            <?php else: ?>
              <form method="POST" action="perbaikan.php" class="inline-form">
                <input type="hidden" name="action" value="vote">
                <input type="hidden" name="id" value="<?= htmlspecialchars($pr['id']) ?>">
                <button type="submit" class="btn btn-secondary btn-sm pr-vote-btn">
  ...
  <div class="card">
    <h3 class="section-title-icon">
      <i class="bi bi-person-fill"></i> Riwayat Laporan Saya
    </h3>
      ...
      <table>
        ...
        <tbody>
          <?php if (empty($myRepairs)): ?>
            <tr><td colspan="5" class="td-empty" style="padding:40px">Tidak ada laporan</td></tr>
          <?php else: ?>
            <?php foreach ($myRepairs as $mr): ?>
              <tr>
                <td><span class="td-mono"><?= htmlspecialchars($mr['id']) ?></span></td>
                <td class="td-bold"><?= htmlspecialchars($mr['target']) ?></td>
